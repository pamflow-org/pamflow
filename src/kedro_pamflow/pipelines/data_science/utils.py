...
def single_spectrogram(species, segments_file_name,plot_params):
    ...
    segments_file_name=os.path.join(segments_path,
                                    '_'.join(species.split()), 
                                     segments_file_name
                                     )
...
def spectrograms_single_species(species, file_names_list, plot_params):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_jobs) as executor:
        ...
        futures = [executor.submit(single_spectrogram,
                                    species, 
                                   segment_file_name,
                                   plot_params
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                print(f"Error processing the {i}th spectrogram for species {species}: {e}")
            i+=1
