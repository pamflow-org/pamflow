...
from kedro_pamflow.pipelines.export.utils import (
...
def from_deployments_to_CSA_eventos(deployments,media,fdm):
    ...
    columnas_CSA_eventos=mandatory_columns_CSA_eventos+non_mandatory_columns_CSA_eventos
    ...
    fdm=fdm[list(fdm_rename_dictionary.keys())+['Hora inicial','Hora final']].rename(columns=fdm_rename_dictionary)
    fdm['EventTime']=fdm['Hora inicial'].astype(str)+' | '+fdm['Hora final'].astype(str)
    fdm=fdm.drop(columns=['Hora inicial','Hora final'])

    
    media=media[list(media_rename_dictionary.keys())+['deploymentID','fileLength']].rename(columns=media_rename_dictionary)
    deployments=deployments[list(deployments_rename_dictionary.keys())+['deploymentID']].rename(columns=deployments_rename_dictionary)
    ...
    media['Duration(HH:MM:SS)']=media.groupby('deploymentID')['fileLength'].transform('sum')
    deployments=deployments.merge(media.drop(columns=['fileLength']), 
                                  on='deploymentID',
                                  how='left'
                                  )
    ...
    CSA_eventos=deployments.merge(fdm,
                                  on='FieldNumber',
                                  how='left'
    ...
    columnas_fdm=['Nombre de la carpeta proyecto (NOMBRE_AÑO)', 'Indicador de evento',
       'Fecha inicial', 'Fecha final', 'País', 'Departamento', 'Municipio',
       'Localidad', 'Latitud', 'Longitud', 'Numero de archivos',
       'Nombre del responsable', 'Apellido  del responsable',
       'Equipo de grabación', 'Definiciones!D17', 'Elevación',
       'Calidad de grabación', 'Nombre del proyecto', 'Comentario de sonido',
       'Duración de cada grabación', 'Configuración de muestreo', 'Hábitat',
       'Área Natural Protegida', 'Nombre del instalador',
       'Apellido  del instalador', 'Publicado', 'Estrato de Vegetación']
