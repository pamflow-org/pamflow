...
class ImageFolderDataset(AbstractDataset[Dict[str, Any], Dict[str, Any]]):
    ...
    def _load(self):
            ...
            for root, dirs, files in os.walk(subfolder_path):
                for file in files:
                    if file.lower().endswith(".png"):
                        ...
                        png_file_name = os.path.split(png_file_path)[-1].replace(
                            ".png", ""
                        )
        ...
        for subfolder_name, sub_dict in png_paths_dict.items():
            partitioned_dataset = [
                (png_file_name, MatplotlibWriter(filepath=png_file_path))
                for png_file_name, png_file_path in sub_dict.items()
            ]
